...
use openalex_core::enrich::enrich_one;
use openalex_core::extract::{extract_index_lookup, extract_rows_to_parquet};
use openalex_core::index::{build_index_shard, concat_index_shards};
use openalex_core::manifest::{
    local_dir_for, manifest_url_to_local, ManifestEntity, ParquetManifest,
};
use openalex_core::parquetio::{
    parquet_distinct_strings, parquet_full_rowcount, parquet_rowcount_meta,
    parquet_top_level_columns,
};
...
/// Fetch the top-level parquet manifest via `aws s3 cp <uri> -` and parse it
/// (parsing lives in `openalex_core::manifest`).
