...
references the other:
  ...
  2. PROFILES config     (./openalex-snapshot.performance.yaml)
     ...
     - Pointed at by the global `--profiles-config <path>` flag.
...
struct ConfigArgs {
    ...
    #[arg(long, default_value_t = false)]
    #[arg(
        help = "Write the PROFILES config YAML (default ./openalex-snapshot.performance.yaml). Auto-derives a `stratified-<RAM_GB>` profile from the host's detected RAM. This is a SEPARATE file from --create; `--profiles-config` points at it. Custom profiles defined here override the built-in `safe` / `stratified-36` and are selected with `convert --profile <name>`."
    )]
    create_profiles: bool,

    #[arg(long, default_value_t = false)]
    #[arg(
        help = "List all profiles visible to the binary: built-ins (`safe`, `stratified-36`) plus any loaded from --profiles-config. Shows each profile's strata table."
    )]
    list_profiles: bool,
...
/// `performance.yaml` are merged on top via `profile_registry`.
...
/// `performance.yaml` (loaded via `--profiles-config` or the default sibling-config
...
/// Writes to `--profiles-config <path>` (or `./openalex-snapshot.performance.yaml`).
...
- For other RAM tiers, supply a user-defined profile in `openalex-snapshot.performance.yaml` (sibling to the main config or via `--profiles-config`).
